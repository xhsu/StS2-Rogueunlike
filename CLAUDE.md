...
| `PotionRewardPickerPatch.cs`, `RelicRewardPickerPatch.cs` | Features #2/#3: pick-then-claim on `NRewardButton.GetReward`. Predetermined rewards stay vanilla (potion side: `Populate` roll-witness; relic side: `_predeterminedRelic`) — EXCEPT roll-built predetermined items: relics (Toy Box, Neow's Bones) bracket-tagged with their roll's pool (wax mark carried onto the pick), and potions (feature #2's missing half, gap closed in v0.6.0: Potion Courier's Ransack, Wellspring, The Legends Were True, Battleworn Dummy, Endless Conveyor's condiment) via `MethodType.Async` handler brackets — MoveNext-level, so rolls after an `await` still tag. Never consume bags manually — the claim's `RelicCmd.Obtain` does. |
