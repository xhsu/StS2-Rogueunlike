...
| `ModWireCheck.cs` | MP handshake: `rl_wirecheck <version> <pickId> <assignId> <designateId>` networked console cmd announced at run start (GenerateRooms/room-entry/first-rewards triggers); `Verified` ⟺ every player announced a matching tuple; `SyncReady` gates reward pickers, shop shading/assign, chest expansion and the Ancient pickers — unverified real MP stays pure vanilla. Exists because mod `INetMessage` wire ids follow per-machine mod LOAD ORDER (`ModManager.SortModList` ties break on local settings). |
...
| `TreasureChestPickerPatch.cs` | Feature #3.1: shaded treasure table; round-based picking over the deterministically expanded shared vote list; vanilla RPS fights; losers re-pick. Mid-round rewards overlays (relic-granted potions) keep reporting `SharedRelicPicking` and z-lift the hands above the overlay — the MP hand stays the one pointer (OS cursor hidden) for the whole chest phase; pause/menus untouched. |
...
| `AncientStartDesignatePatch.cs` | Feature #5.2: designate the act-START Ancient's options (act 1 auto-enters Neow — no map click, so feature #5's seam never fires). `NEventRoom._Ready` postfix opens the same picker modal (pool-of-one; the WHO is decided) over the opening dialogue once the wirecheck + async event Begin settle (raw SceneTree timer — `Cmd.Wait` no-ops under FastMode=Instant); confirm swaps the LIVE event's `_currentOptions`+`GeneratedOptions` in place (each wrapper-slot's roll located on the page BY REFERENCE — other mods append options, e.g. BaseLib's Neow interactions; same Resolve/pool validation as #5.1) and rebuilds the option rows via the room's own `SetOptions` — dialogue keeps playing; RNG untouched (roll already happened). MP: choices sync BY INDEX as direct messages (`OptionIndexChosenMessage`), which the lockstep cmd queue can't order against — so the swap rides the SAME stream (`AncientDesignateMessage` on `EventSynchronizer`'s buffer, per-sender FIFO; broadcast-then-apply in one frame; own patch group, failure ⇒ `ForceBroken` like reward-net); early designates buffer and drain at `AncientEventModel.SetInitialEventState`. Zero save writes — reload replays vanilla and simply offers again. |
| `ModAncientPickerUi.cs` | "Select an Ancient" modal over the map screen (features #5) and over the act-start event room (feature #5.2): row list (map icon, name, epithet, home act) + side info panel listing every option the clicked Ancient can offer (hover tips clipped — the tip column caps at viewport height); any unlocked row is inspectable, only pickable rows select; vanilla roll pre-selected. |
