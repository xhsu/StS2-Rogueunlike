...
| `ShowAllCardRewardsPatch.cs` | Feature #1, state: card rewards generate the whole valid pool through the vanilla per-card pipeline. Fixed-list rewards built from witnessed internal rolls (Kaleidoscope's foreign-pool picks) expand the same way at the ctor; hand-built lists (tutorial) stay vanilla. `WitnessedRolls` here is the shared origin-gate + expansion recipe the off-screen choice seams (ChooseFromRollPatches) reuse. |
...
| `ModRewardScreenUi.cs`, `ModCardPickModal.cs` | Card-picker hosts: reward-screen body swap / THE unified modal host. `ModCardPickModal` has two mounts: in-screen (merchant slots, mod dim, dies with the shop) and `NOverlayStack` overlay (choose-a-card seams — vanilla shared backstop + `NetScreenType.CardSelection`, exactly the layering of the vanilla screen it stands in for; never mount pickers at the raw tree root). Back button = cancel (shop) / skip (choose flows, only when the vanilla call site allows skipping — a vanilla-legal empty-handed outcome). |
| `ChooseFromRollPatches.cs` | Feature #1's missing half (gap closed in v0.6.0): witnessed-roll expansion at the two card-choice seams that bypass the rewards screen — these were always inside #1's "pick any card the roll could produce" promise. `FromChooseACardScreen` (Lead Paperweight kin): ≤3 after expansion stays the vanilla fan (arg swap), >3 replaces the call with the exact vanilla choice protocol around `ModCardPickModal` (overlay mount). `FromSimpleGridForRewards` (Brain Leech, Cheese): single-origin arg swap, `MaxSelect ≤ rolled` reachability gate (Sea Glass's per-rarity quotas excluded; Sealed Deck self-expands via feature #1). Both `Priority.Last` + `__runOriginal`-honoring so other mods' wrappers (SteamRandomMatchMod's ChoiceSync) compose. |
| `PotionRewardPickerPatch.cs`, `RelicRewardPickerPatch.cs` | Features #2/#3: pick-then-claim on `NRewardButton.GetReward`. Predetermined rewards stay vanilla (potion side: `Populate` roll-witness; relic side: `_predeterminedRelic`) — EXCEPT roll-built predetermined items: relics (Toy Box, Neow's Bones) bracket-tagged with their roll's pool (wax mark carried onto the pick), and potions (feature #2's missing half, gap closed in v0.6.0: Potion Courier's Ransack, Wellspring, The Legends Were True, Battleworn Dummy, Endless Conveyor's condiment) via tag calls transpiled into the handlers' MoveNext IL after each `newobj PotionReward` (v0.6.1 — a ctor postfix gets inlined away inside Harmony-patched bodies; see GAME-API-SURFACE § event-potion-rolls). Never consume bags manually — the claim's `RelicCmd.Obtain` does. |
