...
| `NOverlayStack.Push/Remove` + `IOverlayScreen`/`IScreenContext` (+ `NetScreenType.CardSelection`) | Vanilla modal layering for the choose-a-card picker: shared backstop, input isolation, stacking, MP screen-type reporting |
...
| Target | Kind | Note |
|---|---|---|
| `CardFactory.CreateForReward(Player,int,CardCreationOptions)` | prefix (nameof, typed overload) | THE pool replacement. Gated on `CardCreationFlags.IsCardReward` — set by `CardReward.Populate` AND (2026-07-05 audit) `SealedDeck.ChooseCards`, whose pick-10-of-30 draft therefore already expands to pick-10-of-pool (reachable: any 10 distinct cards fit in one 30-distinct roll) |
| same method | postfix | Origin witness for NON-IsCardReward rolls (weak table card→options; result is a materialized List — iterating cannot re-roll). `WitnessedRolls` is the shared origin-gate + expansion recipe consumed by the fixed-list ctor below and the off-screen choice seams |
...
| `CardSelectCmd.FromChooseACardScreen` | prefix (nameof, `Priority.Last`, honors `__runOriginal`) | Feature #1's missing half, seam A (Lead Paperweight, Hefty Tablet, Massive Scroll — gap closed in v0.6.0): witnessed-roll expansion. ≤3 after expansion swaps the arg (vanilla fan hosts it — the method throws above 3); >3 replaces the call with the vanilla choice protocol verbatim — `PlayerChoiceSynchronizer.ReserveChoiceId/SyncLocalChoice/WaitForRemoteChoice`, `PlayerChoiceResult.FromIndex/AsIndex`, `SignalPlayerChoiceBegun(None)/Ended`, `NPlayerHand.CancelAllCardPlay`, `LocalContext.IsMe` + `NetService.Type != Replay` — around `ModCardPickModal` pushed on `NOverlayStack` (`IOverlayScreen`, shared backstop, `NetScreenType.CardSelection` — vanilla layering; raw-root mounting renders with no backstop); local UI failure degrades to `NChooseACardSelectionScreen.ShowScreen/CardsSelected` with the roll (indexes stay valid: expanded starts with the originals) |
| `CardSelectCmd.FromSimpleGridForRewards` | prefix (nameof, `Priority.Last`, honors `__runOriginal`) | Feature #1's missing half, seam B (Brain Leech pick-1, Room Full of Cheese pick-2 — gap closed in v0.6.0): single-origin witnessed arg swap; `CardSelectorPrefs.MaxSelect ≤ rolled count` reachability gate (multi-origin per-rarity quotas — Sea Glass — excluded); the vanilla grid + `FromIndexes` protocol run unchanged on the bigger list. Re-entrant calls are idempotent: extras aren't witnessed |
...
adds `Rarity == Uncommon`). The pick substitution and MP sync reuse the feature #2 flow
(`reward.Potion = choice.ToMutable()` + `RewardPickMessage`) unchanged.
...
2. **Pool-derivation mirrors** (selection = loot law): `ModRelicPickerUi.ValidDrops` ↔
   ...
   `PotionRewardPicker.EventPool` ↔ the five bracketed events' pool LINQ.
3. **Protocol mirror**: `ChooseACardExpandPatch.PickFlow` ↔ the body of
   `CardSelectCmd.FromChooseACardScreen` call-for-call (reserve → signal begun → local
   pick / remote wait → sync FromIndex → signal ended). If MegaCrit reshapes that
   protocol (a new sync call, a changed result encoding), re-mirror here — drift shows
   as MP choice desyncs or a hung "waiting for player" state on remotes.
4. **Flow replacement** (intentional, not a mirror): `TreasureChestPicker.ResolveRound`
